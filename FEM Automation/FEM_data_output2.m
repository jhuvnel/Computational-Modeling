...
tic
sel_post_crista_bnd = 'geom1_sel7'; % posterior canal crista
sel_lat_crista_bnd = 'geom1_sel5'; % lateral canal crista
sel_ant_crista_bnd = 'geom1_sel6'; % anterior canal crista
sel_sacc_crista_bnd = 'geom1_sel3'; % saccule crista
sel_utr_crista_bnd = 'geom1_sel4'; % utricle crista


dset_vest = 'dset1'; % tag for the dataset containing vestibular nerve flow vector field
sel_vest_dom = 1; % vestibular nerve domain selection for 20230104_otolith_edit model 

basisVec1TagsVest = {'cc.e1x','cc.e1y','cc.e1z'}; % basis vector 1 (along the axon)
basisVec2TagsVest = {'cc.e2x','cc.e2y','cc.e2z'}; % basis vector 2 (orthogonal to other two vectors)
basisVec3TagsVest = {'cc.e3x','cc.e3y','cc.e3z'}; % basis vector 3 (orthogonal to other two vectors)
basisVecTagsVest = [basisVec1TagsVest, basisVec2TagsVest, basisVec3TagsVest];
...
[traj_post_pre, p0_post] = fiberGenComsolv2(flow_vest_fixed, flow_post_crista, locIndex_SCC, step, basisVecTagsVest, model, dset_vest);
disp('Trajs done with posterior canal.')
[traj_lat_pre, p0_lat] = fiberGenComsolv2(flow_vest_fixed, flow_lat_crista, locIndex_SCC, step, basisVecTagsVest, model, dset_vest);
disp('Trajs done with lateral canal.')
[traj_ant_pre, p0_ant] = fiberGenComsolv2(flow_vest_fixed, flow_ant_crista, locIndex_SCC, step, basisVecTagsVest, model, dset_vest);
...
[traj_sacc_pre, p0_sacc] = fiberGenComsolv2(flow_vest_fixed, flow_sacc_crista, locIndex_utr_sacc, step, basisVecTagsVest, model, dset_vest);
[traj_utr_pre, p0_utr] = fiberGenComsolv2(flow_vest_fixed, flow_utr_crista, locIndex_utr_sacc, step, basisVecTagsVest, model, dset_vest);
...
numAxons_5to8 = parameterCellSCC_5to8{1}(1);
numAxons_6to10 = parameterCellSCC_6to10{1}(1);
tempSCC = cumsum([numAxons_1to3, numAxons_2to5, numAxons_3to5, numAxons_4to7, numAxons_5to8, numAxons_6to10]);
...
for i = 1:3
    ...
    bigParamCellSCC{i,1} = parameterCellSCC_1to3;
    bigParamCellSCC{i,2} = parameterCellSCC_2to5;
    bigParamCellSCC{i,3} = parameterCellSCC_3to5;
    bigParamCellSCC{i,4} = parameterCellSCC_4to7;
    bigParamCellSCC{i,5} = parameterCellSCC_5to8;
    bigParamCellSCC{i,6} = parameterCellSCC_6to10;
%     % now put in the correct axons this parameter cell will refer to
    ...
    bigParamCellSCC{i,1}{6} = rand(bigParamCellSCC{i,1}{1}(1),1);
    bigParamCellSCC{i,2}{6} = rand(bigParamCellSCC{i,2}{1}(1),1);
    bigParamCellSCC{i,3}{6} = rand(bigParamCellSCC{i,3}{1}(1),1);
    bigParamCellSCC{i,4}{6} = rand(bigParamCellSCC{i,4}{1}(1),1);
    bigParamCellSCC{i,5}{6} = rand(bigParamCellSCC{i,5}{1}(1),1);
    bigParamCellSCC{i,6}{6} = rand(bigParamCellSCC{i,6}{1}(1),1);
end
...
numAxons_3to7 = parameterCellUS_3to7{1}(1);
numAxons_6to10 = parameterCellUS_6to10{1}(1);
tempUS = cumsum([numAxons_1to4, numAxons_3to7, numAxons_6to10]);
...
for i = 4:5 % iterator for toKeep
    ...
    % first copy parameter cells
    bigParamCellUS{ii,1} = parameterCellUS_1to4;
    bigParamCellUS{ii,2} = parameterCellUS_3to7;
    bigParamCellUS{ii,3} = parameterCellUS_6to10;
    % now put in the correct axons this parameter cell will refer to
    ...
    % now put in random initial conditions
    bigParamCellUS{ii,1}{6} = rand(bigParamCellUS{ii,1}{1}(1),1);
    bigParamCellUS{ii,2}{6} = rand(bigParamCellUS{ii,2}{1}(1),1);
