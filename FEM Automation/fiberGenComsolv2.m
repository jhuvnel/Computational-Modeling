...
step_stream_in = [50e-3; -1];
[streamVerts, step_stream] = stream3Comsol(V_nerve.p,V_nerve.t,V_nerve.d1,V_nerve.d2,...
    V_nerve.d3,seedNodes3d,step_stream_in);
...
success_length = 4.5; % mm

trajs = cell(numGen, 3);

for i = 1:numGen
    if size(streamVerts{i},2) < ceil(success_length/step_stream{i}(1))
        trajs{i,3} = [0;0;0];
        trajs{i,2} = [];
        warning(['Current trajectory ',num2str(i),' is too short. Filling traj with zeros.'])
    else
        arclen = arclength(streamVerts{i}(1,:),streamVerts{i}(2,:),streamVerts{i}(3,:));
        ...
        try
            ...
            trajs{i,3} = interparc(relArcLen,streamVerts{i}(1,:),streamVerts{i}(2,:),streamVerts{i}(3,:))';
            % output step vector
            trajs{i,2} = step_full;
        catch
