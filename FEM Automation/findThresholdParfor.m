...
fileDate = date;
saveDir = 'R:\Computational Modeling\Model as of 20230623\Results 20230816\';
...
%%% ONLY VESTIBULAR NERVES %%%
nBandSCC = 6;
nBandUS = 3;
nSol = length(sol_post);
nNerve = 5; % number of nerves - 5 vestibular 
nAxonGroups = (3*nBandSCC + 2*nBandUS); % update based on how many nerves are being simulated - 3 SCC, 2 oto, 1 facial, 1 cochlear
nSims = nAxonGroups*nSol;
% set up all cells for parfor
% resultsThresh_all_parfor = cell(1,nSims);
simClass_all = cell(1,nSims);

sol_all = [sol_post, sol_lat, sol_ant, sol_sacc, sol_utr];
paramToSolMap = zeros(1,nSims);
% sol_all = cell(1,nSims);
...
trajColors = {'g','r','b',[0.8500 0.3250 0.0980],[165,42,42]/245,'c',[0.4940 0.1840 0.5560]};
...
%% Subplot Figure with all monopolar (distant ref) electrodes
% Creates activation curves (all nerves on same axis). Each electrode has
% its own figure.



fMonopolar = figure('Units','Inches','Position',[1 0.5 14 8]);
for i = 1:11
    ph = cell(7,1);
    subplot(3,4,i)

    %%% Log x-axis
    ph{1} = semilogx(currVector,  antSumNorm{i},'LineWidth',1,'Color',trajColors{1}); 
    hold on
    ph{2} = semilogx(currVector,  latSumNorm{i},'LineWidth',1,'Color',trajColors{2}); 
    ph{3} = semilogx(currVector,  postSumNorm{i},'LineWidth',1,'Color',trajColors{3}); 
    ph{4} = semilogx(currVector,  saccSumNorm{i}, '--','LineWidth',1,'Color',trajColors{4}); 
    ph{5} = semilogx(currVector,  utrSumNorm{i}, '--','LineWidth',1,'Color',trajColors{5}); 
%     ph{6} = semilogx(currVector,  facSumNorm{i}, ':','LineWidth',1,'Color',trajColors{6}); 
%     ph{7} = semilogx(currVector,  cochSumNorm{i}, ':','LineWidth',1,'Color',trajColors{7});
...
%     uistack(ph{8},'bottom')
    ...
    %%% Linear x-axis
%     ph{1} = plot(currVector,  antSumNorm{i},'LineWidth',1,'Color',trajColors{1}); 
%     hold on
%     ph{2} = plot(currVector,  latSumNorm{i},'LineWidth',1,'Color',trajColors{2}); 
%     ph{3} = plot(currVector,  postSumNorm{i},'LineWidth',1,'Color',trajColors{3}); 
%     ph{4} = plot(currVector,  saccSumNorm{i}, '--','LineWidth',1,'Color',trajColors{4}); 
%     ph{5} = plot(currVector,  utrSumNorm{i}, '--','LineWidth',1,'Color',trajColors{5}); 
% %     ph{6} = plot(currVector,  facSumNorm{i}, ':','LineWidth',1,'Color',trajColors{6}); 
% %     ph{7} = plot(currVector,  cochSumNorm{i}, ':','LineWidth',1,'Color',trajColors{7});
% 
%     % plot standard range of used current
% %     ph{8} = area(normalCurrentRange, [1 1]*maxNum, 'FaceColor', [1 1 1]*0.9, 'LineStyle','none');
% %     ph{8} = plot(normalCurrentRange, [1 1]*.97*maxNum, 'Color', [1 1 1]*0.7, 'LineWidth', 3); 
...
%     ylim([0 200])
    % legend
    if i == 11
        legend([ph{1} ph{2} ph{3} ph{4} ph{5}], nerve_names(1:5),'Location','northwest','NumColumns',2) % apply legend to last plot
        % legend([ph{1} ph{2} ph{3} ph{4} ph{5} ph{6} ph{7}], nerve_names,'Location','northwest','NumColumns',2) % apply legend to last plot
        legend('boxoff')
    end
    
    % add title and axis labels
    title(StimElectrodeNames{i});
    xlabel('Current Amplitude [uA]');
    ylabel('% Fibers Activated');
    
    % save figure
    saveas(gcf,[saveDir,'MonopolarSubplotLog_',fileDate])
end
%% Separate figure for each electrode
% Creates activation curves (all nerves on same axis). Each electrode has
% its own figure.
...
%     ylim([0 200])
    ...
    % add title and axis labels
    title(['Stim: ',StimElectrodeNames{i},'    Return: ',RefElectrodeNames{i}]);
    xlabel('Current Amplitude [uA]');
    ylabel('% Fibers Activated');
    
    % save figure
    saveas(gcf,[saveDir,'Stim_',StimElectrodeNames{i},'_Return_',RefElectrodeNames{i},'_',fileDate])
end
