...
public class SENN_AxonP extends Axon implements GlobalConstants {
				...
				if ((numNodes-1)%2 == 0)	{	//even number means active node
					...
					for (int e = 0; e < numEndNodes; e++)	{
						...
						if (e%2 == 0)	{	//even number means passive node
							Node eT = new SENN_PassiveNode(nodeDiam, nodeLength_P, deltaT);
							endNodes[e] = eT;
						}
						else	{
					...
					n = new SENN_PassiveNode(nodeDiam, nodeLength_P, deltaT);
					//now implement the endNodes
					for (int e = 0; e < numEndNodes; e++)	{
						//find out if it should be a passive or active node
						if (e%2 == 0)	{	//even number means passive node
	...
	public void initNodeList(double[] nodeDiam, double[] nodeLength_A, double[] nodeLength_P)	{
		...
		for (int i = 0; i < nodes.length; i++)	{
			...
			if (i == 0)	{
				//create the node
				n = new SENN_ActiveNode(nodeDiam[0], nodeLength_A[0], deltaT);
				...
				if ((numNodes-1)%2 == 0)	{	//even number means active node
					...
					for (int e = 0; e < numEndNodes; e++)	{
						//find out if it should be a passive or active node
						if (e%2 == 0)	{	//even number means passive node
							Node eT = new SENN_PassiveNode(nodeDiam[iteratorD], nodeLength_P[iteratorP], deltaT);
							endNodes[e] = eT;
						}
						else	{
					...
					n = new SENN_PassiveNode(nodeDiam[iteratorD], nodeLength_P[iteratorP], deltaT);
					//now implement the endNodes
					for (int e = 0; e < numEndNodes; e++)	{
						//find out if it should be a passive or active node
						if (e%2 == 0)	{	//even number means passive node
							...
							Node eT = new SENN_PassiveNode(nodeDiam[iteratorD], nodeLength_P[iteratorA], deltaT);
							endNodes[e] = eT;
						}
					}
