...
public class AxonSimulate_G_AHPAxon_CVStar5082 extends AxonSimulate {
	...
	public void initAxonArray(int numIterations, int[] numNodes) throws Exception {
		...
		for (int i = 0; i < axons.length; i++)	{
			...
			axons[i] = new G_AHPAxon_CVStar5082(step, nodeDiam[i], nodeLength_A, nodeLength_P, numNodes[i], timeIncrement, numIterations);
		}
	}
	public Axon initOneAxon(int numIterations, int numNodes, double[] nodeDiameter)	throws Exception {
		Axon r = new G_AHPAxon_CVStar5082(step, nodeDiameter, nodeLength_A, nodeLength_P, numNodes, timeIncrement, numIterations);
		return r;
	}
}