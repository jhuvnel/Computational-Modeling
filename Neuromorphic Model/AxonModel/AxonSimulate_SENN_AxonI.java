...
public class AxonSimulate_SENN_AxonI extends AxonSimulate {
	...
	public void initAxonArray(int numIterations, int[] numNodes) throws Exception {
		...
		for (int i = 0; i < axons.length; i++)	{
			...
			axons[i] = new SENN_AxonI(step, nodeDiam[i], nodeLength_A, nodeLength_P, numNodes[i], timeIncrement, numIterations, null);
		}
	}
	public Axon initOneAxon(int numIterations, int numNodes, double[] nodeDiameter, PrintWriter errLog)	throws Exception {
		Axon r = new SENN_AxonI(step, nodeDiameter, nodeLength_A, nodeLength_P, numNodes, timeIncrement, numIterations, errLog);
		return r;
	}
}
