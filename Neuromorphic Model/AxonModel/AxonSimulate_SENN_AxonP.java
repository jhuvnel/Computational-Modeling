...
public class AxonSimulate_SENN_AxonP extends AxonSimulate {
	...
	public void initAxonArray(int numIterations, int[] numNodes) throws Exception {
		...
		for (int i = 0; i < axons.length; i++)	{
			...
			axons[i] = new SENN_AxonP(step, nodeDiam[i], nodeLength_A, nodeLength_P, numNodes[i], timeIncrement, numIterations);
		}
	}
	public Axon initOneAxon(int numIterations, int numNodes, double[] nodeDiameter)	throws Exception {
		Axon r = new SENN_AxonP(step, nodeDiameter, nodeLength_A, nodeLength_P, numNodes, timeIncrement, numIterations);
		return r;
	}
}
