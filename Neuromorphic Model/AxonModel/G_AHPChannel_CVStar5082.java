 ...
 * presented here are for a node diameter of 2.81e-6m, and a node length of 2e-6m
*/

import java.util.Arrays;
import static java.lang.Math.*;

public class G_AHPChannel_CVStar5082 extends Channel implements GlobalConstants {
	...
	public double gs = 6.5;			//gs = strength of synaptic noise (roughly based off of "A" from smith/goldberg paper)
	//need to get gk0 in terns of unit area also
	public double gk0 = 780;			//constant term for hyperpolarizing potassium current
	public double gk = 780;			//holds the current value of gk
	public double prev_gk = 0;		//stored value of a previous value of gk
	public double tk = 0.00236;			//hyperpolarizing current time constant in seconds (from smith/goldberg paper)
	...
	public double shotNoiseInit = 50;	//what to initialize the shot noise memory too

	//for testing
	public double test;
	
	public G_AHPChannel_CVStar5082(double diameter, double length,
	...
	public double current(double Vm) {
		...
		if (timeLastAP != -1 && ((numIters - timeLastAP)*deltaT) < gk_wait)	{
			test = 0;//(numIters - timeLastAP)*deltaT;
		}
	...
	public int findPoisson(double r)	{
		int result;
		result = Arrays.binarySearch(PoissonDistribution.pD_lam50_dTpt1ms, r);
