...
node = zeros(3,numToGen);

%now begin to generate seed nodes
for i = 1:numToGen
    node(:,i) = g.seedNode(locInd);
    plot3(node(1,i),node(2,i),node(3,i),'-o');
end

figure
plotTriSurf(subBndMatrix, vMatrix, 0, '-g');
for i = 1:numToGen
    q = node(:,i);
    nodeM = g.map3D(q);
    plot3(nodeM(1),nodeM(2),nodeM(3),'-o');
end
